    ...
    <>
      ...
      {sheetStack.length > 0 && (
        <div className="lg:hidden fixed inset-0 z-40 flex flex-col justify-end">
          ...
          />
          <div className="relative bg-white rounded-t-2xl shadow-2xl max-h-[70vh] flex flex-col animate-[slideUp_0.2s_ease-out]">
      ...
      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-30 bg-white border-t border-slate-200 pb-[env(safe-area-inset-bottom)]">
        <div className="grid grid-cols-5 h-14">
          <button
            ...
            className={`flex flex-col items-center justify-center gap-0.5 ${location.pathname === '/dashboard' ? 'text-emerald-600' : 'text-slate-500'}`}
          >
          ...
          {quickEntries.map(entry => (
            <button
              ...
              className={`flex flex-col items-center justify-center gap-0.5 ${isGroupActive(entry) || currentKey === entry.key ? 'text-emerald-600' : 'text-slate-500'}`}
            >
          ...
          <button
            ...
            className={`relative flex flex-col items-center justify-center gap-0.5 ${currentKey === 'root' ? 'text-emerald-600' : 'text-slate-500'}`}
          >
