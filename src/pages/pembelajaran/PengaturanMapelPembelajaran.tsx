          ...
          {Object.entries(grouped).map(([grupMapel, items]) => (
            <div key={grupMapel} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
              <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
                <h3 className="text-sm font-bold text-gray-800">{grupMapel}</h3>
              </div>
                ...
                {items.map(m => (
                  <div key={m.id} className="flex items-center justify-between px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">{m.kodeMapel} &middot; {m.aktifPembelajaran ? 'Aktif' : 'Tidak Aktif'}</p>
                    </div>
                    <button
                      onClick={() => toggleMutation.mutate({ id: m.id, aktifPembelajaran: !m.aktifPembelajaran })}
                      className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-800 focus:ring-offset-2 ${
                    ...
                    >
                      <span
                        ...
                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                          m.aktifPembelajaran ? 'translate-x-5' : 'translate-x-0'
                        }`}
                      />
                    </button>
                  </div>
