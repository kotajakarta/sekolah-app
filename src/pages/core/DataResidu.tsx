...
import { AlertTriangle, CheckCircle2, XCircle, Search, RefreshCw, Filter, LayoutDashboard, Layers, Users } from 'lucide-react';
import apiClient from '../../lib/apiClient';
import AdvancedFilterBar, { FilterState } from '../../components/AdvancedFilterBar';
import Pagination from '../../components/Pagination';
import ResiduDashboardTab, { ResiduStudent } from '../../features/core_data/components/ResiduDashboardTab';
import ResiduEmisVervalSubTab from '../../features/core_data/components/ResiduEmisVervalSubTab';
import { useAuth } from '../../hooks/useAuth';

export default function DataResidu() {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'data'>('dashboard');
  const [dataSubTab, setDataSubTab] = useState<'esantri' | 'emis_verval'>('esantri');
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2 p-1.5 bg-slate-100/90 rounded-2xl w-fit border border-slate-200/80 shadow-xs">
            ...
            >
              ...
              <span>Residu eSantri</span>
              <span className="ml-1 text-xs px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 font-bold">
                {filteredStudents.length}
              </span>
            </button>

            <button
              onClick={() => setDataSubTab('emis_verval')}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
                dataSubTab === 'emis_verval'
                  ? 'bg-white text-indigo-700 shadow-sm font-bold'
                  : 'text-slate-600 hover:text-slate-900 hover:bg-white/60'
              }`}
            >
              <Layers className="w-4 h-4" />
              <span>Residu EMIS / VERVAL (Komparasi)</span>
              <span className="ml-1 text-[11px] px-2 py-0.5 rounded-full bg-rose-100 text-rose-700 font-bold">
                Sinkronisasi Kemenag & Kemendikbud
              </span>
            </button>
          </div>
          ...
          {dataSubTab === 'esantri' ? (
            ...
            <ResiduEmisVervalSubTab
              userScope={user?.scope}
              userCabangId={user?.cabangId}
              userWilayahId={user?.wilayahId}
              selectedCabangId={advancedFilters.cabangId}
              selectedWilayahId={advancedFilters.wilayahId}
            />
          )}
        </div>
      )}
